...
function orange_preprocess_page(&$vars){
	...
	$template_files_alias = _template_suggestions('alias', $alias);
	$template_files_normal = _template_suggestions('normal', $normal);
	
	$temp_files_all = array_merge($template_files_alias, $template_files_normal);
	
	$vars['template_files'] = (array_key_exists('template_files', $vars)) ? $vars['template_files'] : array();
	
	$vars['template_files'] = array_merge($vars['template_files'], $temp_files_all);
}

function _template_suggestions($which, $path){
	...
	foreach($path_array as $path_part){
		...
		$template_files[] = $template_filename;
	}
...
function _path_to_array($string){
	$array = split('/', $string);
	$array = array_filter($array, '_reject_empty');
	return $array;
}

function _reject_empty($string){
	return $string !== '';
}
